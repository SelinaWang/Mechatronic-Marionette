...
void loop() {
  ...
  if (mode == 0) {  // Right hand raise
      if (mode0counter == 0) {
        currentMillis = millis();
      if (currentMillis - previousMillis > 5000) {            // wait for two second
      ...
      if (mode0counter == 1) {
        currentMillis = millis();
      if (currentMillis - previousMillis > 15000) {            // wait for two second
      previousMillis = currentMillis;
      mode0counter++;
      Serial.println(mode0counter);
  ...
  else if (mode == 1) {  // Right hand raise
      if (mode1counter == 0) {
        currentMillis = millis();
      if (currentMillis - previousMillis > 5000) {            // wait for two second
      previousMillis = currentMillis;
    ...
    if (mode2counter == 0) {
        currentMillis = millis();
      if (currentMillis - previousMillis > 8000) {            // wait for two second
      previousMillis = currentMillis;
      mode2counter++;
    righthandMotor->run(RELEASE);
    lefthandMotor->run(RELEASE);
    ...
    if (mode3counter == 0) {
        currentMillis = millis();
      if (currentMillis - previousMillis > 10000) {            // wait for two second
      previousMillis = currentMillis;
      mode3counter++;
    righthandMotor->run(FORWARD);
    lefthandMotor->run(RELEASE);
