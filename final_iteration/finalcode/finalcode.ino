...
Modes: 0: STOP, 1: Wave, 2: bow, 3: jump, 4: march, 5: hip drop dance, 6: drop dead, 7: shopping cart dance, 8: fly, 9: sprinkler
*/
...
//    leftshoulderMotor->run(RELEASE);
    bottomMotor->run(RELEASE);
       ...
       if (mode2counter == 4) {
        currentMillis = millis();
      if (currentMillis - previousMillis > 400) {            // wait for two second
      previousMillis = currentMillis;
      mode2counter++;
    righthandMotor->run(BACKWARD);
    lefthandMotor->run(BACKWARD);
...
//    leftshoulderMotor->run(RELEASE);
      bottomMotor->run(BACKWARD);
        ...
        if (mode3counter == 4) {
        currentMillis = millis();
      if (currentMillis - previousMillis > 1050) {            // wait for two second
      previousMillis = currentMillis;
      mode3counter++;
    righthandMotor->run(RELEASE);
    lefthandMotor->run(RELEASE);
...
//    leftshoulderMotor->run(RELEASE);
      bottomMotor->run(RELEASE);
            ...
            if (mode3counter == 5) {
        currentMillis = millis();
      if (currentMillis - previousMillis > 150) {            // wait for two second
      previousMillis = currentMillis;
      mode3counter++;
    righthandMotor->run(RELEASE);
    lefthandMotor->run(RELEASE);
...
//    leftshoulderMotor->run(RELEASE);
    ...
    if (mode3counter == 6) {
        currentMillis = millis();
      if (currentMillis - previousMillis > 50) {            // wait for two second
      previousMillis = currentMillis;
      mode3counter++;
    righthandMotor->run(BACKWARD);
    lefthandMotor->run(BACKWARD);
...
//    leftshoulderMotor->run(RELEASE);
    ...
    if (mode3counter == 7) {
        currentMillis = millis();
      if (currentMillis - previousMillis > 1450) {            // wait for half a second
      previousMillis = currentMillis;
      mode3counter = 0;
...
//    leftshoulderMotor->run(RELEASE);
    ...
    if (mode4counter == 2) {
        currentMillis = millis();
      if (currentMillis - previousMillis > 670) {            // wait for two second
      previousMillis = currentMillis;
      mode4counter++;
    lefthandMotor->run(FORWARD);
    righthandMotor->run(RELEASE);
...
//    leftshoulderMotor->run(RELEASE);
      bottomMotor->run(RELEASE);
        ...
        if (mode4counter == 12) {
        currentMillis = millis();
      if (currentMillis - previousMillis > 600) {            // wait for two second
      previousMillis = currentMillis;
      mode4counter++;
    righthandMotor->run(RELEASE);
    lefthandMotor->run(BACKWARD);
...
//    leftshoulderMotor->run(RELEASE);
    ...
    if (mode4counter == 13) {
        currentMillis = millis();
      if (currentMillis - previousMillis > 100) {            // wait for half a second
      previousMillis = currentMillis;
      mode4counter = 0;
      righthandMotor->run(RELEASE);
      lefthandMotor->run(RELEASE);
...
//    leftshoulderMotor->run(RELEASE);
    ...
    if (mode5counter == 5) {
        currentMillis = millis();
      if (currentMillis - previousMillis > 450) {            // wait for two second
      previousMillis = currentMillis;   
      mode5counter++;
    lefthandMotor->run(BACKWARD);
    righthandMotor->run(BACKWARD);
...
//    leftshoulderMotor->run(RELEASE);
      bottomMotor->run(RELEASE);
        ...
        if (mode5counter == 6) {
        currentMillis = millis();
      if (currentMillis - previousMillis > 2800) {            // wait for two second
      previousMillis = currentMillis;   
      mode5counter++;
    lefthandMotor->run(RELEASE);
    righthandMotor->run(RELEASE);
...
//    leftshoulderMotor->run(RELEASE);
    ...
    if (mode5counter == 7) {
        currentMillis = millis();
      if (currentMillis - previousMillis > 25) {            // wait for half a second
      previousMillis = currentMillis;
      mode5counter = 0;
...
//      leftshoulderMotor->run(RELEASE);
      ...
      if (mode8counter == 8) {
        currentMillis = millis();
      if (currentMillis - previousMillis > 1800) {            // wait for 3 second
      previousMillis = currentMillis;
      mode8counter = 0;
      righthandMotor->run(RELEASE);
      lefthandMotor->run(RELEASE);
